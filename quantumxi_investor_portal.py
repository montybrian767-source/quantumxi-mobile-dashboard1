...
.qxi-hero-card {
    background:linear-gradient(135deg,rgba(90,167,255,.18),rgba(34,211,155,.09)),#1b2a39;
    border:1px solid rgba(139,173,202,.22);border-radius:20px;padding:18px;
    box-shadow:inset 0 1px 0 rgba(255,255,255,.04);
}
.qxi-big {font-size:56px;font-weight:950;color:white;letter-spacing:-.08em;}
.qxi-score {font-size:44px;font-weight:950;color:#5aa7ff;letter-spacing:-.06em;}
...
def shell_open():
    ...
    <div class="qxi-shell"><div class="qxi-topbar">
      ...
      <div class="qxi-title">QuantumXI Investor Portal</div><div class="qxi-subtitle">AI market intelligence command center</div>
...
def home():
    ...
    cols=[x for x in ["symbol","decision","consensus_score","live_score","adaptive_score","historical_score","risk_score","reason"] if x in data.columns]
    st.dataframe(data[cols].head(12), use_container_width=True, hide_index=True)
    shell_close()

def top_ideas():
    data=df(); shell_open(); st.markdown("### Top Ideas")
...
def watchlist():
    data=df(); shell_open(); st.markdown("### Watchlist")
    wl=st.session_state.watchlists.setdefault(st.session_state.email,[])
    if not wl:
        st.info("No symbols added yet. Add symbols from Top Ideas."); shell_close(); return
...
def reports():
    data=df(); shell_open(); st.markdown("### Research Reports")
...
def admin():
    shell_open(); st.markdown("### Admin Console")
...
def placeholder(name):
    shell_open(); st.markdown(f"### {name}")
    st.info(f"{name} module placeholder. This can connect to holdings, funds, discovery tools, or news feeds in the next build.")
    shell_close()
